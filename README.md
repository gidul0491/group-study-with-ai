...
`/`는 항상 로그인 화면이다. 회원가입으로 관리자 계정을 만들고(여러 명 가능), 시험지는 만든 계정만 관리한다. 브라우저를 닫으면 로그아웃된다.
...
|------|------|
| `/` | 로그인 |
| `/signup` | 회원가입 (비밀번호 찾기 질문·답변 등록) |
| `/recover` | 비밀번호 찾기 (아이디 → 질문 → 답변 → 새 비밀번호) |
| `/admin` | 내 시험지 목록 |
...
| `/admin/settings` | 공개 주소(QR·링크에 쓰는 host), 기본 프롬프트(새 시험지 공통 프롬프트에 미리 채워짐), 비밀번호 변경, 비밀번호 찾기 질문 등록, 로그아웃 |
